...
export const SHORTCUTS: readonly ShortcutSpec[] = [
  ...
  // --- Terminal -----------------------------------------------------------
  {
    ...
    surface: 'terminal',
    label: 'Paste into the shell',
    defaults: ['Ctrl+Shift+V'],
    owner: 'app',
    rebindable: true,
    note: 'The only chord that has ever pasted into this pane, and the one the terminal’s own documentation names. Right-click does the same thing. Measured: without preventDefault() this fires twice, once from us and once from Chromium’s own paste-as-plain-text.',
  },
  {
  ...
  },
  {
    ...
    owner: 'app',
    rebindable: true,
    note: 'Affordable only because it was measured: plain Ctrl+V in this pane produced a single SYN byte through xterm’s own ctrl-letter mapping and pasted nothing. What it does cost is readline’s literal-next (quoted-insert); Ctrl+Q is bound to the same command in vi mode. Ctrl+Alt+V is left alone — Ctrl+Alt is how AltGr arrives on European layouts, where V sits under a printable character.',
  },
  {
