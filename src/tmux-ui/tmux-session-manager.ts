...
export class TmuxSessionManager {
  ...
  /** Subscription handle from TmuxClient 'event' emission. */
  private clientEventSub: ((event: any) => void) | null = null;

  /** SSH channel for tmux control mode (passed to start). */
   ...
   */
  private onPaneAdded(paneId: string): void {
  ...
  private closeAllPaneTerminals(): void {
    for (const [paneId, terminalId] of this.paneToTerminal) {
      this.terminalManager.closeTerminal(terminalId);
    }
    this.paneToTerminal.clear();
    this.terminalToPane.clear();
