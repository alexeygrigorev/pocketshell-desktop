...
/**
 * Sidebar view provider that surfaces the Prompt Composer in the reworked
 * shell. This is a NEW surface distinct from the existing editor WebviewPanel
 * opened by `prompt-composer-commands.ts`: a persistent `WebviewView` pinned to
 * the activity bar that inspects the active tmux pane for an attributed agent
 * session and offers a launcher button that delegates to the existing
 * `pocketshell.promptComposer.open` command with a normalized agent target. It
 * does NOT reimplement any backend logic.
 *
 * Registration is deferred: callers wire `registerPromptComposerSidebar()`
 * into the feature manifest's `register` (or extension.ts) in a later batch.
 */
export class PromptComposerSidebarProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'pocketshell.promptComposer.sidebar';
  ...
  private async handleMessage(message: { action?: string }): Promise<void> {
    ...
    this.model = markOpeningComposer(this.model);
    this.render();
    try {
      const openArgs = buildComposerOpenArgs(this.model);
      if (!openArgs) {
        throw new Error('No agent session is available for the prompt composer.');
      }
      await vscode.commands.executeCommand('pocketshell.promptComposer.open', openArgs);
