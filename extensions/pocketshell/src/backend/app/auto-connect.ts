 ...
 *
 * On app startup, reconnects to `lastHostId` when available, otherwise falls
 * back to the host with the most recent `lastConnectedAt` timestamp.
 *
...
export class AutoConnectService {
  ...
  async init(): Promise<void> {
    ...
    const lastHost = this.findStartupHost(hosts, settings.lastHostId);
  ...
  /**
   * Schedule auto-connect after the current startup call stack returns.
   *
   * This gives callers a chance to subscribe to events after receiving the app
   * context from initializeApp(). Use waitForIdle() in tests that need to
   * observe the scheduled run deterministically.
   */
  startInBackground(): void {
    if (this.backgroundRun) return;
    this.backgroundRun = new Promise((resolve) => {
      setTimeout(() => {
  ...
  /** Await the currently scheduled background run, if any. */
  ...
  /**
   * Find the startup host. The persisted lastHostId is an explicit hint; if it
   * is stale, fall back to the host with the most recent `lastConnectedAt`.
   */
  private findStartupHost(hosts: Host[], lastHostId: number | null): Host | undefined {
    if (lastHostId != null) {
      const explicitHost = hosts.find((host) => host.id === lastHostId);
