...
export class SshTerminalBackend {
  ...
  kill(): void {
    ...
    // Guard: adapter.destroy() may synchronously trigger onExit/onError
    // callbacks which call cleanup() and dispose the emitters, so check
    // before firing into a potentially disposed emitter.
    if (!this._disposed) {
