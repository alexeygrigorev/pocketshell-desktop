...
| `usage --json` | top-level `short_term`/`long_term` objects | rows carry a keyed `windows` map (`5h`/`7d`/`weekly`/`monthly`/`short_term`) and no top-level pair — quse's record moved underneath, while `pocketshell --version` still says 0.4.44 (fixture `v0.4.44-usage-windows.ndjson`) | A row read raw has no `short_term`; a consumer that indexes it throws during render, which blanks the whole usage panel. `parseUsageNdjson` folds the map into the pair. |
