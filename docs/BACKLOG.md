...
| ✅ | Garbled pane under a busy TUI, curable only by connecting to another session and back | `140a837`, `c4aa4e9`, *this pass* |
...
| ✅ | F18's last acceptance criterion — the FILES tree is keyboard-navigable (roving tabindex, arrows clamp, Home/End jump, Enter/Space open); the session panel already had buttons and chords | `9bfc6da` |
...
- **The garble attempts were fixed in the wrong quantity, and only measuring on the fixture said so.** The reconcile loop compared `#{window_height}` against the xterm grid; on tmux 3.4 a status-lined session's window is one row short of its PTY *by design*, so the probe disagreed every tick (a resize-and-repaint storm every interval, forever) — and agreed straight through real drift with the status line off. The quantities that mean something are `#{client_width} #{client_height}` (exactly what `setWindow` set — the thing our grid must equal) and the window pair (the other side's business, never ours to fight for: tmux 3.4 has no portable reclaim — `resize-window -c` does not exist, `refresh-client -C` is control-mode-only). The second lesson is that the user's own workaround named the cure: leaving the tab and coming back fires one `refresh-client`, which is the only repair for a pane garbling at *constant* geometry — invisible to any size comparison — so the loop now repaints on the clock while healthy, and re-joins the session (a fresh client, a reset terminal) when the probe answers `dead` twice, which a swallowed null used to hide.
